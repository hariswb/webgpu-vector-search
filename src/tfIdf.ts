export function buildAcronymRegex(dict: Record<string, string>): RegExp {
  const escaped = Object.keys(dict).map(k => k.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"));
...
export function preprocessText(
  ...
  // 1. Replace acronyms
  text = text.replace(acronymRegex, (match) => acronymDict[match.toUpperCase()] || acronymDict[match] || match);
...
export function tokenize(preprocessed: string): string[] {
...
export function vectorizeQuery(
  ...
  const cleaned = preprocessText(rawText, acronymDict, acronymRegex, stopwords);
  ...
  for (const term in counts) {
    ...
    if (index !== undefined) {
      vec[index] = counts[term] * idf[index];
    }
  }
